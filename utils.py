...
def read_tiker_figi_json() -> dict:
    ...
    try:
        with open("tiker_figi.json", encoding="utf-8") as tiker_figi:
    ...
    try:
        if interval == CandleInterval.CANDLE_INTERVAL_DAY:
            ...
            if (
                ...
                and (tuple_indicator[1] < tuple_indicator[0] < 70)
            ...
            # (last_sma_10_1  < last_sma_10_2 < last_sma_10_3) and (30<last_rsi<prev_rsi) and (last_MACD<prev_MACD)
                ...
                and (30 < tuple_indicator[0] < tuple_indicator[1])
            ...
            # last_sma_10_2<last_sma_10_1)
            if tuple_indicator[9] < tuple_indicator[8]:
                """ВОЗРОСТАНИЕ SMA 10 """
                buy_hour[tiker] = figi
                filter_tiker = FilterTickerDict(
                    tiker=tiker,
            ...
            # (last_sma_10_1<last_sma_10_2
            elif tuple_indicator[8] < tuple_indicator[9]:
                """УБЫВАЮЩИЙ SMA 10 """
        ...
        #     # (цена закрытия < средняя боллинджера) and (prev_MACD_3 < prev_MACD_4) and
        #     # (prev_MACD_3<prev_MACD<last_MACD<0) and (prev_rsi_3<last_rsi<50)
        ...
        #     # (цена закрытия < средняя боллинджера) and
        #     # (last_sma_10_3<last_sma_10_2<last_sma_10_1) and (prev_rsi_3<last_rsi<55)
        #     elif ((tuple_indicator[12] < tuple_indicator[13]) and (
        #             tuple_indicator[10] < tuple_indicator[9] < tuple_indicator[8]) and (
        #                   tuple_indicator[2] < tuple_indicator[0] < 55)):
        #         "SMA 10 ВОЗРАСТАЕТ НО БОЛЛИНДЖЕР ОГРАНИЧИВАЕТ"
        #         buy_15min[tiker] = figi
        #         filter_tiker = FilterTickerDict(tiker=tiker, timeframe=timeframe, action="buy",
        ...
        #     # (средняя боллинджера < цена закрытия) and (prev_MACD_4 < prev_MACD_3) and
        #     # (0<last_MACD<prev_MACD<prev_MACD_3) and (50<last_rsi<prev_rsi_3)
        ...
        #     # (средняя боллинджера < цена закрытия) and
        #     # (last_sma_10_1<last_sma_10_2<last_sma_10_3) and (45<last_rsi<prev_rsi_3)
        #     elif ((tuple_indicator[13] < tuple_indicator[12]) and (
        ...
        #         sale_15min[tiker] = figi
        #         filter_tiker = FilterTickerDict(tiker=tiker, timeframe=timeframe, action="sell",
        #                                         strategy="4_sell_15_min) убывающий SMA10 и rsi > 50",
        #                                         description=f"last_MACD:{round(tuple_indicator[4], 2)}, last_rsi:{round(tuple_indicator[0], 2)}, midBoll:{round(tuple_indicator[13], 2)}")
        #         session.add(filter_tiker)
        #         session.commit()
        #     else:
...
def select_dicts(buy_day: dict, buy_hour: dict, buy_15min: dict, sale_day: dict, sale_hour: dict, sale_15min: dict):
...
def calculation_number_lots(cl, figi: str, tiker: str):
    """РАСЧЕТ КОЛИЧЕСТВА ЛОТОВ НА СУММУ 6600"""
    try:
        ...
        instrument = cl.instruments.get_instrument_by(
        ...
        for money in positions.money:
            ...
            """ПЕРЕДЕЛАЙ УСЛОВИЕ НА 6600"""
            if units > 6600:
                quantity = int(6600 / (current_price * lot_size))
                return quantity
            else:
...
def opredelaem_schag(cl, figi: str, tiker: str) -> Decimal:
    ...
    try:
        instruments: InstrumentsService = cl.instruments
        # Для акций
        instrument = instruments.share_by(id_type=InstrumentIdType.INSTRUMENT_ID_TYPE_FIGI, id=figi).instrument
                ...
                try:
                    ...
                    if execution_price < price_rub * 1.0038 <= current_price < price_rub * 1.008:
                        ...
                        moving_stop_los(
                            cl=cl,
                            figi=position.figi,
                            tiker=position.ticker,
                            quantity=quantity_lots_new,
                            price_rub=Decimal(price_rub),
                            coeff_sl_price=coeff_sl_price,
                            schag=schag,
                        )
...
def process_sale_with_fifo(session: Session, sale: SalesInform):  # Обьявл сесс и нужн строку при продаже new_line_sale
    ...
    try:
        ...
        buys = (
            session.query(BuyInform)
            .filter(and_(BuyInform.figi == sale.figi, BuyInform.quantity_buy > 0))
            .order_by(asc(BuyInform.date_buy))
            .all()
        )  # получаем строчки по figi где кол-во > 0
        if not buys:
    ...
    try:
        ...
        for operate in operations.operations:
            if operate.operation_type == OperationType.OPERATION_TYPE_BUY:  # Фильтруем по покупке
                ...
                figi = operate.figi
                instruments: InstrumentsService = cl.instruments
                tiker = instruments.share_by(
                    id_type=InstrumentIdType.INSTRUMENT_ID_TYPE_FIGI, id=operate.figi
                ).instrument.ticker
                quantity_buy = int(operate.quantity)
                buy_price = float(operate.price.units + operate.price.nano / 1e9)
                commission_buy = abs(
                    round(operate.child_operations[0].payment.units + operate.child_operations[0].payment.nano / 1e9, 2)
                )
                time.sleep(tm)
                ...
                if existing:
                    ...
                    new_line_buy = BuyInform(
                        date_buy=date_buy.replace(microsecond=0),
                        tiker=tiker,
                        operatio_type=operatio_type,
                        quantity_buy=quantity_buy,
                        buy_price=buy_price,
                        commission_buy=commission_buy,
                        figi=figi,
                    )
                    archive_new_line_buy = ArchiveBuyInform(
                        date_buy=date_buy.replace(microsecond=0),
                        tiker=tiker,
                        operatio_type=operatio_type,
                        quantity_buy=quantity_buy,
                ...
                if existing:
                    ...
                    new_line_sale = SalesInform(
                        date_sale=date_sale.replace(microsecond=0),
                        tiker=tiker,
                        operatio_type=operatio_type,
